...
void PerlinNoise::fill_permutations()
{
    ...
    std::copy(m_permutations.begin() + m_size, m_permutations.end(),
            m_permutations.end());
}
...
double PerlinNoise::get_value(double x, double y) 
{
    ...
    d2 = top_left.dot(vec2(dx - 1.0, dy)); 
    double r1 = util::interpolate(d1, d2, dx); 

    d1 = top_left.dot(vec2(dx, dy - 1.0)); 
    d2 = top_left.dot(vec2(dx - 1.0, dy - 1.0)); 
...
{
    return util::get_random_gradient(m_permutations[m_permutations[(x + 1) % m_size] + (y + 1)% m_size]);
}   
